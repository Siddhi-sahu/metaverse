...
const Arena = () => {
    ...
    const handleMove = (newX: number, newY: number) => {
        ...
        const maxCols = Math.floor(canvas.width / 50);
        const maxRows = Math.floor(canvas.height / 50);

        if (newX >= 0 && newX < maxCols && newY >= 0 && newY < maxRows) {
            wsRef.current.send(JSON.stringify({
                type: 'move',
                payload: {
                    x: newX,
    ...
    return (
        <div className="p-4" onKeyDown={handleKeyDown} tabIndex={0}>
            ...
            <div ref={containerRef} className="w-full max-h-[500px] overflow-hidden border rounded-lg">
                <canvas
                    ref={canvasRef}
                    className="bg-white"
                />
